...
para rodar aplicação basta dar um docker-compose up --build
ou npm i + npm run dev
...
class UserManagement extends React.Component {
  ...
  handleNameChange(event) {
    this.state.newUserName = event.target.value; // aqui está alterando o state diretamente,
    //  utilizeo setState para alterar o state igual está fazendo no handleEmailchange
  }
  ...
  addUser() {
    const newUser = {
      id: this.state.users.length + 1, // isso não da certo porque pode gerar Ids repetidos
      name: this.state.newUserName,
      email: this.state.newUserEmail
    };

    this.state.users.push(newUser);
    this.forceUpdate(); // não se deve utilizar o forceUpdate, o indicado é utilizar o setState
      ...
      <div>
        ...
        <div>
          <input
            ...
            value={this.state.newUserName} // aqui está falta o onchange para mudar o estado
          />
          <input
            type="email"
        ...
        <ul>
          {this.state.users.map(user => (
            <li> // aqui falta a key para o map funcionar
...
Resposta: O problema é porque a api retornou um objeto que o front não estava esperando como resposta, assim causando a quebra do component, então devemos inicializar os products com o state com um array vazio, e deve validar utilizando um try/catch para verificar se products existe antes de renderizar na tela para evitar quebra de codigo, assim retornamos um error no state para informar melhor o usuário
...
Bff é um padrão de arquitetura onde cada tipo de interface possui seu proprio backend(TV, Web, Mobile). Os benefícios de implementar um bff são:
- dados sob medida, mobile e tv recebe quantidades menores de dados
- redução de request na rede
- times de desenvolvimento trabalha sem conflitos
- mudanças no bff web não afeta o mobile nem tv
- os MS não fica expostos diretamente 
...
Resposta: Não, porque são plataformas diferentes, o usuário do web se comporta de forma diferente do mobile e da tv, são experiencias diferentes
...
R= CLIENT, porque o local responsável pela renderização acontece no lado do client
...
R= BFF, porque o bff chama o MS de catálago e o ms de usuários
...
R= BFF mobile, porque ele entrega apenas dados necessarios para o mobile
d) Lógica de negócio principal para registrar que um usuário assistiu a um vídeo (atualizar histórico).
R= MICROSERVICE para armazenar a informação do histórico, e entregar para o usuario em diferentes plataformas diferentes 
e) Lógica para adaptar a qualidade do stream de vídeo (MS de Streaming) com base na velocidade da conexão do usuário Mobile.
R= BFF para analisar formato e entregar a qualidade e MS para gerenciar a entrega  
f) Validação de formato de e-mail no formulário de cadastro no cliente Web.  
R= Client,BFF e MICROSERVICE, aqui o ideal é haver validação em cada camada, principalmente no MS
